...
getContentType, downloadContentFromMessage } = require('@adiwajshing/baileys');
 ...
 module.exports = async (
   ...
   function clockString(ms) {
    ...
    MenuList += ` 👋🏻 *Hello:* ${m.pushName}\n`
    MenuList += `💣 *Day:* ${m.sayingtime + m.timoji}\n`
    MenuList += ` 🕒 *Time:* ${time}\n`
    ...
    MenuList += `${LETT++}. ${prefix}ytmp4 link\n`
    MenuList += `${LETT++}. ${prefix}ytmp3 link\n\n`
...
case 'mining': case 'menambang':{
 ...
 case 'owner': case 'creator': {
  ...
  case 'menu2':{  
   let button = [{
     index: 1, 
      urlButton: {
       ...
       url: 'https://github.com/arielsp12'
       } 
     },     
     {
     index: 2, 
   ...
   let buttons = [{
    ...
    await m.createMsg(
     ...
     {
     image: {
      url: './storage/video/ArielKece.jpg', 
  ...
  case 'sewabot':
  imageMsg = (await sock.prepareMessageMedia(fs.readFileSync(`./storage/video/QRByAriel.jpg`), 'imageMessage', {thumbnail: fs.readFileSync(`./storage/video/QRByAriel.jpg`)})).imageMessage
  reply(`*OPEN SEWABOT*\n\n➪ *Permanen :* *_10K_*`)
  break

  case 'youtubemp3': case 'ytmp3':{
  if (args.length < 1) return reply('linknya?')
  reply(`Sedang Di Proses Ngab\n Ariel Kece Abis`)  
   try{
    await yta(args[0])
.then((res) => {
     const { dl_link } = res
...
.then((a) => {
  ...
  case 'ytmp4':{
  if (args.length < 1) return reply('linknya?')
  reply(`Ariel Kece Tunggu Ya Bang Lagi Di Proses`)
   try{
    await ytv(args[0])
.then((res) => {
     const { dl_link } = res
 ...
 break
  ...
  case 'hoorror_blood':{
    if (args.length < 1) return m.reply(from, 'Teks?', { quoted : m } )
     ...
     console.log(anu)
    sock.sendMessage(from, { image: { url : anu }, caption: `Nih kak, Jangan Lupa Donasi\n${Options.info.igowner}`}, { quoted: m } )
